...
class XCADDataset(Dataset):
    def __init__(self, dataroot, split='train'):
        ...
        if split == 'train':
            ...
            if 'XCAD' in dataroot:
                ...
                dataPath = os.path.join(dataroot, 'test','images')
                dataFiles = sorted(os.listdir(dataPath))
            elif '30XCA' in dataroot:
                dataPath = os.path.join(dataroot, 'Images')
                dataFiles = sorted(os.listdir(dataPath))
            elif '134XCA' in dataroot:
                dataPath = os.path.join(dataroot, 'Images')
                dataFiles = sorted(os.listdir(dataPath))
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            ...
            if '134XCA' in self.dataroot:
                dataInfo = self.imageNum[index]
                dataPath = glob.glob(os.path.join(self.dataroot,'Images', dataInfo[:-4]+'*'))[0]
                data_A = cv2.imread(dataPath, cv2.IMREAD_GRAYSCALE).astype('float') / 255.
                labelPath = glob.glob(os.path.join(self.dataroot, 'GT', dataInfo[:-4]+'*'))[0]
                data_F = cv2.imread(labelPath, cv2.IMREAD_GRAYSCALE).astype('float') / 255.

            elif '30XCA' in self.dataroot:
                dataInfo = self.imageNum[index]
                dataPath = glob.glob(os.path.join(self.dataroot,'Images', dataInfo[:-4]+'*'))[0]
                data_A = cv2.imread(dataPath, cv2.IMREAD_GRAYSCALE).astype('float') / 255.
                labelPath = glob.glob(os.path.join(self.dataroot, 'GT', dataInfo[:-4]+'*'))[0]
            ...
            A_path = dataInfo
        [data_A, data_F] = Util.transform_augment([data_A, data_F], split=self.split, min_max=(-1, 1), eq=False)
