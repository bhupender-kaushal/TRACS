...
from pytorch_grad_cam import GradCAM, AblationCAM, GradCAMPlusPlus, ScoreCAM, EigenCAM, EigenGradCAM, LayerCAM, HiResCAM
from pytorch_grad_cam.utils.image import show_cam_on_image
import cv2

class TRICSS(BaseModel):
    def __init__(self, opt):
        super(TRICSS, self).__init__(opt)
        ...
        if opt['path']['resume_state'] is not None and opt['phase']=='train':
            ...
            temp=torch.rand(256,256,dtype=torch.float).unsqueeze(0).cuda()
            feat_f, sample_ids = self.netH(temp, 256, None)
    ...
    def data_dependent_initalize(self, data, opt):
        ...
        _,_, self.mask_V, self.synt_A, self.mask_F = output #
    ...
    def optimize_parameters(self, ce=1):
        h_alpha = 0.2 
        h_beta = 2.0       
        current_epoch=ce
        output, [l_recon, l_l1, l_bce, l_cldice] = self.netG(self.data)

        _,_, self.mask_V, self.synt_A, self.mask_F = output #,
        ce=self.set_device(torch.tensor(ce))
        # (2*l_l1 /(torch.exp(0.02*(ce-1)))+ (torch.exp(0.007*(ce-1))-1)*(0.15*l_bce+0.2*l_cldice)) #l_l1*h_beta #
        l_cyc1 = 2*l_l1  #/(torch.exp(0.02*(ce-1)))# 2*l_l1
        l_cyc2=0.1*l_bce #(torch.exp(0.007*(ce-1))-1)*(0.15*l_bce)#
        l_cyc3=0.1*l_cldice #(torch.exp(0.007*(ce-1))-1)*(0.2*l_cldice)#

        l_cyc=l_cyc1+l_cyc3+l_cyc2
    ...
    ###################training the generators ########################
        ...
        l_tot =  l_cont + l_adv_G  + l_recon +  l_cyc
        l_tot.backward()
        self.optG.step()
        self.optH.step()
        ...
        self.log_dict['l_l1'] = l_cyc1.item()
        self.log_dict['l_bce'] = l_bce.item()
        self.log_dict['l_cldice'] = l_cldice.item()
    ...
    def get_cam(self, data_dict):
        ...
        gaussian_3 = to_tensor(cv2.GaussianBlur(x_img, (0, 0), 5.0)).unsqueeze(0).cuda()
        unsharp_image = to_tensor(cv2.addWeighted(x_img, 2.0, gaussian_3.cpu().numpy().squeeze().squeeze(), -1.0, 0)).unsqueeze(0).cuda()

        input_tensor = torch.cat([a_start, unsharp_image], dim=1) 
        generator_model = GeneratorWrapper(self.netG.segment_fn)
        ...
        target_layer = self.netG.segment_fn.conv_img[1]

        cam_models = [AblationCAM, ScoreCAM, LayerCAM, HiResCAM]
        targets = [MeanOutputTarget()]
        

        import gc
        for cam_class in cam_models:
            cam = cam_class(model=generator_model, target_layers=[target_layer])
            cam_result = cam(input_tensor=input_tensor, targets=targets)
            grayscale_cam.append(cam_result)
            del cam
            del cam_result
            gc.collect()
...
class GeneratorWrapper(nn.Module):
    def __init__(self, segment_fn):
        super().__init__()
        self.segment_fn = segment_fn  # Just a pointer

    def forward(self, x):
        return self.segment_fn(x)
    
class MeanOutputTarget:
    def __call__(self, model_output):
        return model_output.view(model_output.size(0), -1).mean(dim=1)